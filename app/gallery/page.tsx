...
// ⭐ FIX 1 — Define a strict type for gallery items
type GalleryItem = {
  id: number
  type: "image" | "video"
  src: string
...
// ⭐ FIX 2 — Apply the GalleryItem[] type
...
export default function GalleryPage() {
  ...
  // ⭐ FIX 3 — Correct selectedItem type
      ...
      <section className="relative bg-gradient-to-br from-black via-gray-900 to-black text-white py-20 overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
          ...
          >
            <h1 className="text-4xl md:text-6xl font-bold mb-6 font-heading">Gallery</h1>
            <p className="text-xl text-gray-300">
              Explore our state-of-the-art facilities and see what makes TEAM MUSCLE FITNESS
              special
            </p>
          </motion.div>
        </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 px-2 sm:px-4">
              {filteredItems.map((item, index) => (
                ...
                >
                  <div onClick={() => handleItemClick(item)} className="w-full">
                    <CardContainer containerClassName="py-0" className="w-full">
                      <CardBody className="h-full overflow-hidden bg-gray-800 border-2 border-gray-700 hover:border-red-600/70 transition-all duration-300 hover:shadow-2xl hover:shadow-red-600/30 cursor-pointer rounded-xl">
                        <CardItem translateZ="100" rotateX={10} className="p-1.5 w-full">
                          <div className="relative aspect-square overflow-hidden rounded-lg bg-gray-900/50">
                            {item.type === "image" ? (
                              <>
                                <img
